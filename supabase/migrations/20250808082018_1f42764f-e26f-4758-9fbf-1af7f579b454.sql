...
do $$ begin
  create policy "Users can view their own files in drive"
  on storage.objects
  for select
  to authenticated
  using (
...
do $$ begin
  create policy "Users can upload files to their own folder in drive"
  on storage.objects
  for insert
  to authenticated
  with check (
...
do $$ begin
  create policy "Users can update their own files in drive"
  on storage.objects
  for update
  to authenticated
  using (
...
do $$ begin
  create policy "Users can delete their own files in drive"
  on storage.objects
  for delete
  to authenticated
  using (
